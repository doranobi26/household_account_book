class IncomeValuesController < ApplicationController
  ...
  def create
    ...
    if @form.save
      redirect_to income_value_path(@income_value.id), notice: "登録しました"
    else
      redirect_to new_income_values_path, notice: "登録に失敗しました"
    end
  end
  ...
  def update
    ...
    if @income_value.save
      redirect_to income_value_path(@income_value.id), notice: "情報を更新しました"
    else
      render "edit"
    end
  end
